  <div class='error container-error'><div class='error-icon'>
	Antes de prosseguir preencha corretamente o formulário e revise os campos abaixo:</div>
	<ol> 
		<li><label for="galeria" class="error-validate">Envia alguma imagem/foto</label></li> 
		<li><label for="titulo" class="error-validate">Informe o título</label></li> 
		<li><label for="data" class="error-validate">Entre com uma data válida</label></li> 
		<li><label for="preco" class="error-validate">Informe o <b>valor</b> do veículo</label></li> 
		<li><label for="resumo" class="error-validate">Você precisa informar algum <b>resumo</b></label></li> 
		<li><label for="destaque1" class="error-validate">Você precisa informar algum <b>texto</b> para destaque 1</label></li> 
		<li><label for="destaque2" class="error-validate">Você precisa informar algum <b>texto</b> para destaque 2</label></li> 
		<li><label for="estrelas" class="error-validate">Você precisa selecionar o número de <b>estrelas</b> do automóvel</label></li> 
	</ol> 
  </div>



<form method='post' action='?<?=$_SERVER['QUERY_STRING']?>' id='form_<?=$p?>' class='form cmxform' enctype="multipart/form-data">
 <input type='hidden' name='act' value='<?=$act?>'>
<?php
  if ($act=='update')
    echo "<input type='hidden' name='item' value='${_GET['item']}'>";
?>

<h1>
<?php 
  if ($act=='insert') echo $var['insert'];
   else echo $var['update'];
?>
</h1>
<p class='header'>Todos os campos com <b>- * -</b> são obrigatórios.</p>



    <ol>


	<li>	
	  <label>Fotos<span class='small'><a href='javascript:void(0);' class='addImagem' id='min'>adicionar + fotos</a></span></label>
	  <?php
		  
	    if ($act=='update') {
				  
		    $sql_gal = "SELECT rci_id,rci_imagem,rci_pos FROM ".TABLE_PREFIX."_r_${var['pre']}_imagem WHERE rci_${var['pre']}_id=? AND rci_imagem IS NOT NULL ORDER BY rci_pos ASC;"; 
		    $qr_gal = $conn->prepare($sql_gal);
		    $qr_gal->bind_param('s',$_GET['item']);
		    $qr_gal->execute();
		    $qr_gal->bind_result($g_id,$g_imagem,$g_pos);
		    $i=0;

		      echo '<table id="posGaleria" cellspacing="0" cellpadding="2">';
		      while ($qr_gal->fetch()) {
	  ?>
		<tr id="<?=$g_id?>">
		  <td width='20px' title='Clique e arraste para mudar a posição da foto' class='tip'></td>

		  <td class='small'>
		    [<a href='?p=<?=$p?>&delete_galeria&item=<?=$g_id?>&prefix=r_<?=$var['pre']?>_galeria&pre=rci&col=imagem&folder=<?=$var['imagem_folderlist']?>&noVisual' title="Clique para remover o ítem selecionado" class='tip trash-galeria' style="cursor:pointer;" id="<?=$g_id?>">remover</a>]
		  </td>

		  <td>

		    <a href='$imagThumb<?=$i?>?width=100%' id='imag<?=$i?>' class='betterTip'>
		     <img src='images/lupa.gif' border='0' style='background-color:none;padding-left:10px;cursor:pointer'></a>

			 <div id='imagThumb<?=$i?>' style='float:left;display:none;'>
			 <?php 
			 
			    if (file_exists(substr($var['path_thumb'],0)."/".$g_imagem))
			     echo "<img src='".substr($var['path_thumb'],0)."/".$g_imagem."'>";

			       else echo "<center>imagem não existe.</center>";
			  ?>
			 </div>

		  </td>
		</tr>

	      <?php
		      $i++;	

			}
		   echo '</table><br>';

	       }
	       ?>


		 <div class='divImagem'>
		   <input class="galeria <?php /*if ($act=='insert') echo ' required';*/?>" type='file' name='galeria0' id='galeria' alt='0' style="height:18px;font-size:7pt;margin-bottom:8px;">
		   <br><span class='small'>- JPEG, PNG ou GIF;<?=$var['imagemWidth_texto'].$var['imagemHeight_texto']?></span>
		 </div>
		 </p>
        </li>

	<li>	
	  <label>Categoria *<span class='small'>Categoria do produto</span></label>
	  <select name='cat_id' id='cat_id' class='required'>
		<option>Selecione</option>
		<?php
		  $sql_marca = "SELECT cat_titulo, cat_id FROM ".TABLE_PREFIX."_categoria WHERE cat_status=1 AND cat_area='Catalogo'";
		  $qry_marca = $conn->prepare($sql_marca);
		  $qry_marca->bind_result($nome, $id);
		  $qry_marca->execute();
		 
			  while ($qry_marca->fetch()) {
		?>
		   <option value='<?=$id?>'<?php if ($act=='update' && $val['cat_id']==$id) echo ' selected';?>> <?=$nome?></option>
		<?php } $qry_marca->close(); ?>
	  </select>
	</li>

	<li>	
	  <label>Código *<span class='small'>Código do produto</span></label>
	  <input type='text' name='codigo' id='codigo' class='required' value='<?=$val['codigo']?>'>
	</li>

	<li>
	  <label>Data *<span class='small'>Entre com a data de cadastro (para ordenação)</span></label>
	  <input type='text' id='data' name='data' class='required' value='<?=dateen2pt('-',$val['data'],'/')?>'>
	  <br/>
	</li>

	<li>	
	  <label>Título *<span class='small'>Nome do produto</span></label>
	  <input type='text' name='titulo' id='titulo' class='required' value='<?=$val['titulo']?>'>
	</li>


	<li>	
	  <label>Valor *<span class='small'>Preço do produto</span></label>
  	  <?php $valor = empty($val['valor']) ? null : Moeda($val['valor']);?>
	  <input type='text' name='valor' id='valor' class='required' value='<?=$valor?>'>
	</li>

	<li>
	  <label>Resumo *<span class='small'>Resumo para a listagem</span></label>
	  <textarea name='resumo' id='resumo' class='required' cols='80' rows='4'><?=stripslashes($val['resumo'])?></textarea>
	</li>

	<li>
	  <label>Descrição/Texto *<span class='small'>Informações do produto</span></label>
	  <textarea name='descricao' id='descricao' class='required' cols='80' rows='6'><?=stripslashes($val['descricao'])?></textarea>
	</li>

	<li>
	  <label>Observação<span class='small'>Para promoção e etc.</span></label>
	  <textarea name='obs' id='obs' cols='80' rows='2'><?=stripslashes($val['obs'])?></textarea>
	</li>

	<li>	
	  <label>Mostrar na home? *<span class='small'>Sim ou não</span></label>
	  <select name='home' id='home' class='required'>
		<option value=1 <?php if($val['home']==1) echo ' selected';?>>Sim</option>
		<option value=0 <?php if($val['home']==0) echo ' selected'; elseif($act=='insert') echo ' selected'; ?>>Não</option>
	  </select>
	</li>

 </ol>



    <br>
    <p align='center'>
    <input type='submit' value='ok' class='first'><input type='button' id='form-back' value='voltar'></p>
    <div class='spacer'></div>


</form>


